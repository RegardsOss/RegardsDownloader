...
@HostPlugin(revision = "$Revision: 33350 $", interfaceVersion = 2, names = { "hugefiles.net" }, urls = { "https?://(www\\.)?hugefiles\\.net/((vid)?embed\\-)?[a-z0-9]{12}" }, flags = { 2 })
    ...
    private void controlHost(final Account account, final DownloadLink downloadLink, final boolean action) throws Exception {
        synchronized (CTRLLOCK) {
            ...
            if (!action) {
                ...
                // Improvised errorhandling for known XFS3 bug
                if (usedSlots == null) {
