...
@HostPlugin(revision = "$Revision: 33357 $", interfaceVersion = 3, names = { "imgtown.net" }, urls = { "https?://(www\\.)?imgtown\\.net/(?:embed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class ImgtownNet extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                     = "http://imgtown.net";
    private static final String            NICE_HOST                       = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty               = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                         = "(imgtown\\.net)";
    ...
    public ImgtownNet(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (dllink == null && IMAGEHOSTER) {
            ...
            do {
                ...
                } else if (correctedBR.contains("name=\"next\"")) {
                    /* Special */
                    String postData = "op=view&id=" + this.fuid + "&pre=1&next=Continue+to+Image...";
                    final String specialthing = new Regex(correctedBR, "name=\"([^<>\"])\" value=\"1\"").getMatch(0);
                    if (specialthing != null) {
                        postData += "&" + specialthing + "=1";
                    }
                    this.postPage(this.br.getURL(), postData);
                }
