...
@HostPlugin(revision = "$Revision: 32094 $", interfaceVersion = 3, names = { "media1fire.com" }, urls = { "https?://(www\\.)?up\\.media1fire\\.com/((vid)?embed\\-)?[a-z0-9]{12}" }, flags = { 0 })
...
public class MediaOneFileCom extends PluginForHost {
     ...
     * */
    public MediaOneFileCom(PluginWrapper wrapper) {
        super(wrapper);
        setConfigElements();
    ...
    private String checkDirectLink(final DownloadLink downloadLink) {
        ...
        if (dllink != null) {
            try {
                ...
                if ((con.getContentType().contains("html") || con.getLongContentLength() == -1) && !con.isContentDisposition()) {
                    downloadLink.setProperty(directlinkproperty, Property.NULL);
                    dllink = null;
                }
                con.disconnect();
