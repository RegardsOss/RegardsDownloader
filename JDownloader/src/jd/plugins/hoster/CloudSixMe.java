...
@HostPlugin(revision = "$Revision: 33848 $", interfaceVersion = 2, names = { "cloudsix.me" }, urls = { "https?://(www\\.)?cloudsix\\.me/(embed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class CloudSixMe extends antiDDoSForHost {
    ...
    private static final String  COOKIE_HOST                  = "http://cloudsix.me";
    private static final String  NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String  NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String  DOMAINS                      = "(cloudsix\\.me)";
    ...
    public CloudSixMe(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, "(No such file|>File Not Found<|>The file was removed by|Reason for deletion:\n|File Not Found|>The file expired)").matches() || this.br.getHttpConnection().getResponseCode() == 403 || this.br.getHttpConnection().getResponseCode() == 404) {
    ...
    private String getFnameViaAbuseLink(final Browser br, final DownloadLink dl) throws Exception {
        getPage("http://" + NICE_HOST + "/?op=report_file&id=" + fuid);
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        final String space[] = new Regex(correctedBR, ">Used space:\\s*<b>([0-9\\.]+) ?(KB|MB|GB|TB)?</b>").getRow(0);
        ...
        final String availabletraffic = new Regex(correctedBR, "Traffic available.*?:\\s*<b>([^<>\"\\']+)</b>").getMatch(0);
