...
@HostPlugin(revision = "$Revision: 32094 $", interfaceVersion = 3, names = { "junocloud.me" }, urls = { "https?://(\\w+\\.)?junocloud\\.me/((vid)?embed-)?[a-z0-9]{12}" }, flags = { 2 })
@SuppressWarnings("deprecation")
public class JunoCloudMe extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://junocloud.me";
    // domain names used within download links.
    private final String               DOMAINS                      = "(junocloud\\.me)";
    ...
    // other: no redirects
    // mods: action required for download1 and download2 forms, account checking expire, requestFileInformation other file not found regex
     ...
     * */
    public JunoCloudMe(PluginWrapper wrapper) {
        super(wrapper);
        setConfigElements();
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        final String space[] = cbr.getRegex(">Used space:</div>.*?>([0-9\\.]+) of \\d+ ?(KB|MB|GB|TB)?").getRow(0);
        ...
        String availabletraffic = cbr.getRegex("Traffic available.*?:</div>.*?>([^<>\"']+)").getMatch(0);
        ...
        if (account.getBooleanProperty("free")) {
            ...
            getPage(br.getURL() + "?op=payments"); // '/home.html' also has it.
            ...
            if (inValidate(expireDay)) {
                getPage("/home.html");
                expireDay = cbr.getRegex("(\\d{1,2} (January|February|March|April|May|June|July|August|September|October|November|December) \\d{4})").getMatch(0);
            }
            if (!inValidate(expireDay)) {
    ...
    private synchronized void setHashedHashKeyValue(final Account account, final Integer x) {
        ...
        if (!hostMap.isEmpty()) {
            ...
            /* Workaround attempt for old XFS3 bug: http://svn.jdownloader.org/issues/43719 */
