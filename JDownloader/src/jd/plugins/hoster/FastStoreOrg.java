...
@HostPlugin(revision = "$Revision: 33325 $", interfaceVersion = 2, names = { "faststore.org" }, urls = { "https?://(www\\.)?faststore\\.org/(embed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class FastStoreOrg extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                  = "http://faststore.org";
    private static final String            NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                      = "(faststore\\.org)";
    ...
    public FastStoreOrg(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (fileInfo[1] == null && SUPPORTS_ALT_AVAILABLECHECK) {
            // fileInfo contains wrong filesize
        ...
        synchronized (LOCK) {
            try {
                ...
                if (loginform.containsHTML("/captchas/")) {
                    ...
                    logger.info("Put captchacode " + c + " obtained by captcha metod \"Re Captcha\" in the form and submitted it.");
                } else if (loginform.containsHTML(";background:#ccc;text-align")) {
                    logger.info("Detected captcha method \"plaintext captchas\" for this host");
                    /* Captcha method by ManiacMansion */
                    final String[][] letters = new Regex(loginform.getHtmlCode(), "<span style='position:absolute;padding-left:(\\d+)px;padding-top:\\d+px;'>(&#\\d+;)</span>").getMatches();
                    if (letters == null || letters.length == 0) {
                        logger.warning("plaintext captchahandling broken!");
                        throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT);
                    }
