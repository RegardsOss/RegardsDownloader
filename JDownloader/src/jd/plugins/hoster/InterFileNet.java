...
@HostPlugin(revision = "$Revision: 32094 $", interfaceVersion = 3, names = { "interfile.net" }, urls = { "https?://(www\\.)?interfile\\.net/((vid)?embed\\-)?[a-z0-9]{12}" }, flags = { 2 })
@SuppressWarnings("deprecation")
public class InterFileNet extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://interfile.net";
    // domain names used within download links.
    private final String               DOMAINS                      = "(interfile\\.net)";
     ...
     * */
    public InterFileNet(PluginWrapper wrapper) {
        super(wrapper);
        setConfigElements();
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        final String space[] = cbr.getRegex(">Disk usage:</td>.*?<td.*?b>([0-9\\.]+) ?(KB|MB|GB|TB)?</b>").getRow(0);
        ...
        String availabletraffic = cbr.getRegex("Available traffic.*?:</TD><TD[^>]+><b>([^<>\"']+)</b>").getMatch(0);
