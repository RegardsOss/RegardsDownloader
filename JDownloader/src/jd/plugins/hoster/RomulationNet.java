...
@HostPlugin(revision = "$Revision: 33814 $", interfaceVersion = 3, names = { "romulation.net" }, urls = { "https?://(?:www\\.)?romulation\\.net/rom/[^/]+/[^/]+" }, flags = { 0 })
public class RomulationNet extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        if (filename == null) {
            filename = new Regex(link.getDownloadURL(), "/([^/]+)$").getMatch(0) + ".7z";
        }
    ...
    private void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (dllink == null) {
            ...
            if (dllink == null) {
                dllink = br.getRegex("\"(/roms/download/guest/[^<>\"]+)\"").getMatch(0);
            }
            if (dllink == null) {
