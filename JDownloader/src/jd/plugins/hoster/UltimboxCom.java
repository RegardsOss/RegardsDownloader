...
@HostPlugin(revision = "$Revision: 32094 $", interfaceVersion = 2, names = { "ultimbox.com" }, urls = { "https?://(www\\.)?ultimbox\\.com/mega/(?!index\\.[a-z]+)[A-Za-z0-9]+" }, flags = { 0 })
public class UltimboxCom extends PluginForHost {
    ...
    public String getAGBLink() {
        return mainpage + "/mega/terms." + type;
    }

    /* Basic constants */
    private final String         mainpage                                     = "http://ultimbox.com";
    private final String         domains                                      = "(ultimbox\\.com)";
    ...
    /* Known errors */
    private static final String  url_ERROR_SIMULTANDLSLIMIT                   = "e=Vous+avez+atteint+le+maximum+de+";
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        if (available_CHECK_OVER_INFO_PAGE) {
            ...
            filename = br.getRegex("(?:Filename|Nom De Fichier):[\t\n\r ]*?</td>[\t\n\r ]*?<td(?: class=\"responsiveInfoTable\")?>([^<>\"]*?)<").getMatch(0);
            ...
            filesize = br.getRegex("(?:Filesize|Taille Du Fichier):[\t\n\r ]*?</td>[\t\n\r ]*?<td(?: class=\"responsiveInfoTable\")?>([^<>\"]*?)<").getMatch(0);
        } else {
