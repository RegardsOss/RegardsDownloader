...
@HostPlugin(revision = "$Revision: 33554 $", interfaceVersion = 2, names = { "redfile.eu" }, urls = { "https?://(www\\.)?redfile\\.eu/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class RedFileEu extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                  = "http://redfile.eu";
    private static final String            NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                      = "(redfile\\.eu)";
    ...
    // mods:
    // limit-info: accountlimits untested, set FREE limits
    // protocol: no https
    // captchatype: solvemedia
    // other: new domain: filepile.eu (YetiShare script)
    ...
    public RedFileEu(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        final String availabletraffic = new Regex(correctedBR, "\"glyphicon glyphicon-signal\"></span>(.*?)</div>").getMatch(0);
    ...
    private void login(final Account account, final boolean force) throws Exception {
        synchronized (LOCK) {
            try {
                ...
                if (!new Regex(correctedBR, "(Premium(\\-| )Account expire|>\\s?Renew premium<)").matches()) {
