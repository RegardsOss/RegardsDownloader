...
@HostPlugin(revision = "$Revision: 33584 $", interfaceVersion = 3, names = { "tusfiles.net" }, urls = { "https?://(www\\.)?tusfil(es\\.(net|com|co\\.nz)|\\.es)/((vid)?embed-)?[a-z0-9]{12}" }, flags = { 2 })
...
public class TusFilesNet extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws Exception {
        ...
        if (redirect != null && redirect.matches(dllinkRegex)) {
            dllink = redirect;
        ...
        if (cbr.containsHTML("The file you are trying to download is no longer available")) {
            throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
        }
    ...
    private void checkServerErrors() throws NumberFormatException, PluginException {
        ...
        if ("78b7afcbdc973998cd635b90bfb50060".equals(JDHash.getMD5(cbr.toString()))) {
