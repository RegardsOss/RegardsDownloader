...
@HostPlugin(revision = "$Revision: 28916 $", interfaceVersion = 2, names = { "video.mediaset.it" }, urls = { "http://(www\\.)?video\\.mediaset\\.it/(video/[^<>/\"]*?/[^<>/\"]*?/\\d+/[^<>/\"]*?\\.html|player/playerIFrame\\.shtml\\?id=\\d+)" }, flags = { 0 })
