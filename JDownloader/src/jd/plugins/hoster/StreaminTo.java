...
@HostPlugin(revision = "$Revision: 32094 $", interfaceVersion = 2, names = { "streamin.to" }, urls = { "https?://(www\\.)?streamin\\.to/(?:(?:vid)?embed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class StreaminTo extends antiDDoSForHost {
    ...
    private String decodeDownloadLink(final String s) {
        ...
        if (decoded != null) {
            ...
            finallink = new Regex(decoded, "(\"|')(https?://[^<>\"']*?\\.(mp4|flv))\\1").getMatch(1);
        }
        return finallink;
    }
