...
@HostPlugin(revision = "$Revision: 33002 $", interfaceVersion = 2, names = { "ulshare.se", "musickapoz.se" }, urls = { "https?://(www\\.)?(musickapoz|ulshare)\\.se/(embed\\-)?[a-z0-9]{12}", "REGEX_NOT_POSSIBLE_RANDOM-asdfasdfsadfsdgfd32424" }, flags = { 0, 0 })
public class MusickapozSe extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                  = "http://ulshare.se";
    private static final String            NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                      = "(ulshare\\.se)";
    ...
    public String rewriteHost(String host) {
        if ("musickapoz.se".equals(getHost())) {
            if (host == null || "musickapoz.se".equals(host)) {
                return "ulshare.se";
            }
        }
    ...
    public MusickapozSe(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (dllink == null && VIDEOHOSTER && downloadLink.getName().endsWith(".mp3")) {
            try {
                logger.info("Trying to get link via vidembed");
                final Browser brv = br.cloneBrowser();
                brv.getPage("/mp3embed-" + fuid + ".mp3");
    ...
    private void waitTime(long timeBefore, final DownloadLink downloadLink) throws PluginException {
        ...
        if (ttt == null) {
            ttt = new Regex(correctedBR, "id=\"countdown_str\">Wait <span id=\"[a-z0-9]+\">(\\d+)</span> seconds</span>").getMatch(0);
        }
        if (ttt != null) {
