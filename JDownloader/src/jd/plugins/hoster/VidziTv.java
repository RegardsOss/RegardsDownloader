...
@HostPlugin(revision = "$Revision: 33318 $", interfaceVersion = 2, names = { "vidzi.tv" }, urls = { "https?://(www\\.)?vidzidecrypted\\.tv/((vid)?embed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class VidziTv extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                  = "http://vidzi.tv";
    private static final String            NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                      = "(vidzi\\.tv)";
    ...
    public final static String  DownloadSubTitles         = "DownloadSubTitles";
    public final static boolean DownloadSubTitles_default = false;

    private void setConfigElements() {
        getConfig().addEntry(new ConfigEntry(ConfigContainer.TYPE_CHECKBOX, getPluginConfig(), DownloadSubTitles, JDL.L("plugins.hoster.VidziTv.downloadsubtitles", "Download subtitles? (when available)")).setDefaultValue(DownloadSubTitles_default));
    }
    ...
    public String[] scanInfo(final String[] fileInfo) {
        ...
        if (fileInfo[0] == null) {
            fileInfo[0] = new Regex(correctedBR, "<h2 class=\"video\\-title\">([^<>\"]*?)</h2>").getMatch(0);
        }
        // if (fileInfo[1] == null) {
