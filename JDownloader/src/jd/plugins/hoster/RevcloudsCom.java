...
@HostPlugin(revision = "$Revision: 32941 $", interfaceVersion = 3, names = { "revclouds.com" }, urls = { "https?://(www\\.)?revclouds\\.com/(?:embed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class RevcloudsCom extends antiDDoSForHost {
    ...
    @SuppressWarnings("deprecation")
    public RevcloudsCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private String[] scanInfo(final String[] fileInfo) {
        final String sharebox0 = "href=[^>]+>\\s*([^\r\n]+) - ([\\d\\.]+ (?:B|KB|MB|GB))</a></textarea>[\r\n\t ]+</div>";
        final String sharebox1 = "\\[URL[^\\]]+\\]\\s*(.*?) - ([\\d\\.]+ (?:B|KB|MB|GB))\\[/URL\\]";
        ...
        if (fileInfo[0] == null) {
            fileInfo[0] = new Regex(correctedBR, "align=\"absmiddle\"><br />([^<>\"/]*?)\\(<span").getMatch(0);
        }
        if (fileInfo[0] == null) {
            fileInfo[0] = new Regex(correctedBR, "\\&fn=([^<>\"]*?)").getMatch(0);
        }
