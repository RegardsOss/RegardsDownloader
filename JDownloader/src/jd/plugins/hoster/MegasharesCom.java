 ...
 */
@HostPlugin(revision = "$Revision: 33589 $", interfaceVersion = 2, names = { "megashares.com" }, urls = { "http://[\\w\\.]*?(d[0-9]{2}\\.)?megashares\\.com/(.*\\?d[0-9]{2}=[0-9a-zA-Z]{7}|dl/[0-9a-zA-Z]{7}/)" }, flags = { 2 })
public class MegasharesCom extends PluginForHost {

    private final String            UserAgent   = "JD_" + "$Revision: 33589 $";
    ...
    public void handleFree(DownloadLink downloadLink) throws Exception {
        ...
        if (!dl.getConnection().isContentDisposition()) {
            ...
            if (br.containsHTML("Get a link card now")) {
                throw new PluginException(LinkStatus.ERROR_IP_BLOCKED, 5 * 60 * 1000l);
            }
            if (br.containsHTML("Your Passport needs to")) {
