...
@HostPlugin(revision = "$Revision: 33358 $", interfaceVersion = 2, names = { "gboxes.com" }, urls = { "https?://(www\\.)?gboxes\\.com/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class GBoxesCom extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                  = "http://gboxes.com";
    ...
    // mods:
    // limit-info: all types unlimited
    // protocol: no https
    // captchatype: null
    // other:
    ...
    public GBoxesCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private void waitTime(long timeBefore, final DownloadLink downloadLink) throws PluginException {
        ...
        if (ttt == null) {
            ttt = new Regex(correctedBR, "id=\"countdown\">[^<>\"]+<span(?: id=\"[^<>\"]+\")?(?: class=\"[^<>\"]+\")?>\\s*(\\d+)\\s*</span>").getMatch(0);
            if (ttt == null) {
                ttt = "60";
            }
        }
