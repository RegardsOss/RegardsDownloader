...
@HostPlugin(revision = "$Revision: 32094 $", interfaceVersion = 2, names = { "rapidvideo.tv" }, urls = { "https?://(www\\.)?rapidvideo\\.(?:tv|org)/((vid)?embed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class RapidVideoTv extends antiDDoSForHost {

    private String                         correctedBR                  = "";
    private String                         passCode                     = null;
    private static final String            PASSWORDTEXT                 = "<br><b>Passwor(d|t):</b> <input";
    // primary website url, take note of redirects
    private static final String            COOKIE_HOST                  = "http://rapidvideo.org";
    private static final String            NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    // domain names used within download links.
    private static final String            DOMAINS                      = "(rapidvideo\\.(?:org|tv))";
    ...
    public void correctDownloadLink(final DownloadLink link) {
        setFUID(link);
        link.setUrlDownload("http://www.rapidvideo.org/" + this.fuid);
    }

    @Override
    public String rewriteHost(String host) {
        if ("rapidvideo.tv".equals(getHost())) {
            if (host == null || "rapidvideo.tv".equals(host)) {
                return "rapidvideo.org";
            }
        }
    ...
    public RapidVideoTv(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, "(No such file|>File Not Found<|>The file was removed by|Reason for deletion:\n)").matches() || !correctedBR.contains("name=\"usr_login\"")) {
