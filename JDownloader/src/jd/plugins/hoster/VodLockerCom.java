...
@HostPlugin(revision = "$Revision: 33357 $", interfaceVersion = 3, names = { "vodlocker.com", "vodlocker.city" }, urls = { "https?://(?:www\\.)?vodlocker\\.com/((vid)?embed-)?[a-z0-9]{12}", "https?://(?:www\\.)?vodlocker\\.city/video/\\d+/[^/]+" }, flags = { 0, 0 })
...
public class VodLockerCom extends PluginForHost {
    ...
    private void getDllink() {
        ...
        if (inValidate(dllink) || (!inValidate(dllink) && !dllink.matches(dllinkRegex))) {
            ...
            if (inValidate(dllink) && cbr.containsHTML("\"rtmp://")) {
                String a = cbr.getRegex("image:\\s*\"(https?://[^/]+/)").getMatch(0);
                String b = cbr.getRegex("(vod|\\.mp4)\\?h=([a-z0-9]{58})\"").getMatch(1);
                if (!inValidate(a) && !inValidate(b)) {
                    dllink = a + b + "/video.mp4";
                }
            }
            if (inValidate(dllink)) {
    ...
    private void checkErrors(final DownloadLink theLink, final Account account, final boolean checkAll) throws NumberFormatException, PluginException {
        ...
        if (cbr.containsHTML("<div id=\"over_player_msg\">Available video slots for this region is full\\.<")) {
            throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "No available slots for this region.", 5 * 60 * 1000l);
        }
    }
