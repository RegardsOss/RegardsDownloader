...
@HostPlugin(revision = "$Revision: 32094 $", interfaceVersion = 3, names = { "gorillavid.in" }, urls = { "https?://(www\\.)?gorillavid\\.in/((vid)?embed-)?[a-z0-9]{12}" }, flags = { 0 })
@SuppressWarnings("deprecation")
public class GorillaVidIn extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://gorillavid.in";
    // domain names used within download links.
    private final String               DOMAINS                      = "(gorillavid\\.in)";
     ...
     * */
    public GorillaVidIn(PluginWrapper wrapper) {
        super(wrapper);
        setConfigElements();
    ...
    private void waitTime(final long timeBefore, final DownloadLink downloadLink) throws PluginException {
        ...
        if (inValidate(ttt)) {
            ttt = cbr.getRegex(">Wait <span id=\"[a-z0-9]+\">(\\d+)</span> seconds<").getMatch(0);
        }
        if (!inValidate(ttt)) {
