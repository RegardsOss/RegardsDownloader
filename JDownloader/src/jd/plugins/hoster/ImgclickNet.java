...
@HostPlugin(revision = "$Revision: 33002 $", interfaceVersion = 2, names = { "imgclick.net" }, urls = { "https?://(www\\.)?imgclick\\.net/(embed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class ImgclickNet extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                  = "http://imgclick.net";
    private static final String            NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                      = "(imgclick\\.net)";
    ...
    public ImgclickNet(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (dllink == null) {
            ...
            if (download1 != null) {
                ...
                sendForm(download1);
                final String specialfilename = br.getRegex("align=\"center\" size=\"18\">([^<>\"]*?)</h4>").getMatch(0);
                if (specialfilename != null) {
                    downloadLink.setFinalFileName(Encoding.htmlDecode(specialfilename).trim());
                }
                checkErrors(downloadLink, false);
                dllink = getDllink();
            }
    ...
    private String getDllink() {
        ...
        if (dllink == null) {
            ...
            if (dllink == null) {
                dllink = new Regex(correctedBR, "(\"|\\')(https?://([\\w\\-\\.]+\\.)?" + DOMAINS + "/i/[^<>\"]*?)(\"|\\')").getMatch(1);
            }
            if (dllink == null) {
