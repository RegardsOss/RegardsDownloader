...
@HostPlugin(revision = "$Revision: 32094 $", interfaceVersion = 3, names = { "vshare.eu" }, urls = { "https?://(www\\.)?vshare\\.eu/((vid)?embed\\-)?[a-z0-9]{12}" }, flags = { 0 })
@SuppressWarnings("deprecation")
public class VShareEu extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://vshare.eu";
    // domain names used within download links.
    private final String               DOMAINS                      = "(vshare\\.eu)";
     ...
     * */
    public VShareEu(PluginWrapper wrapper) {
        super(wrapper);
        setConfigElements();
            ...
            if (inValidate(fileInfo[0])) {
                ...
                if (inValidate(fileInfo[0])) {
                    ...
                    if (inValidate(fileInfo[0])) {
                        ...
                        if (inValidate(fileInfo[0])) {
                            fileInfo[0] = cbr.getRegex("Videoname:</b>  ?(<[^>]+> ?)+?([^<>\"']+)").getMatch(1);
